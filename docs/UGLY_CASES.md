...
| 7 | Unresolved weekday | "Thursday" with no date | Resolve to a calendar date and read it back before committing | `test_weekday_resolved_and_read_back` |
...
| 10 | Recap fails | SMS send errors after agreement | `RECAP_FAILED / NOT_COMMITTED` | `test_recap_failure_blocks_commitment` |
| 11 | Missing audio offset | Commitment with no timestamp anchor | `EVIDENCE_MISSING`, never `verified` | `test_missing_offset_is_not_verified` |
